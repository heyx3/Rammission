...
{
	public static Vector2 GetInput(int playerID) { return (playerID >= playerInputs.Count) ? Vector2.zero : playerInputs[playerID]; }
		...
		for (int i = 0; i < GameSettings.NPlayers; ++i)
		{
			...
			switch (GameSettings.PlayerControlSchemes[i])
			{
				case GameSettings.ControlSchemes.WASD:
					playerInputs[i] = new Vector2((Input.GetKey(KeyCode.D) ? 1 : 0) -
													  (Input.GetKey(KeyCode.A) ? 1 : 0),
												  (Input.GetKey(KeyCode.W) ? 1 : 0) -
												      (Input.GetKey(KeyCode.S) ? 1 : 0));
					break;
				case GameSettings.ControlSchemes.ArrowKeys:
					playerInputs[i] = new Vector2((Input.GetKey(KeyCode.RightArrow) ? 1 : 0) -
													  (Input.GetKey(KeyCode.LeftArrow) ? 1 : 0),
												  (Input.GetKey(KeyCode.UpArrow) ? 1 : 0) -
												      (Input.GetKey(KeyCode.DownArrow) ? 1 : 0));
					break;
				case GameSettings.ControlSchemes.IJKL:
					playerInputs[i] = new Vector2((Input.GetKey(KeyCode.L) ? 1 : 0) -
